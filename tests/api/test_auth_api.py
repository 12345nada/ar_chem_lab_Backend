...
URL prefix is auto-detected from the live app routes so this file
works regardless of whether your router uses /register, /api/register,
/auth/register, /api/v1/register, etc.
"""
import uuid
import pytest
...
def _detect_prefix() -> str:
    """
    Inspect the mounted routes to find where /register lives.
    Returns the prefix string (e.g. '' or '/api' or '/api/v1' or '/auth').
    """
    from app.main import app
    for route in app.routes:
        path = getattr(route, "path", "")
        if path.endswith("/register"):
            return path[: -len("/register")]
    # fallback — print all routes to help debugging
    print("\n[test_auth_api] WARNING: /register not found. All routes:")
    for route in app.routes:
...
REGISTER_URL = f"{PREFIX}/register"
VERIFY_URL   = f"{PREFIX}/verify-email"
LOGIN_URL    = f"{PREFIX}/login"
REFRESH_URL  = f"{PREFIX}/refresh"
PROFILE_URL  = f"{PREFIX}/profile"
FORGOT_URL   = f"{PREFIX}/forgot-password"
RESET_URL    = f"{PREFIX}/reset-password"
